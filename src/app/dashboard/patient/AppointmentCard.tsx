...
import { Calendar, Clock, Stethoscope, Video, AlertCircle, Hourglass, User as UserIcon, FilePenLine } from "lucide-react";
...
export const AppointmentCard = ({ 
  ...
  const handleLeaveReview = () => {
    ...
    router.push('/dashboard/patient/profile?tab=reviews');
  }
  ...
  return (
    <Card className="shadow-sm overflow-hidden flex flex-col">
      ...
      <CardContent className="p-4 flex-grow">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex items-center gap-2 text-sm">
                ...
                <span>{new Date(appointment.date + 'T00:00:00').toLocaleDateString('es-AR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', timeZone: 'America/Argentina/Buenos_Aires' })}</span>
            </div>
            <div className="flex items-center gap-2 text-sm">
      ...
      <CardFooter className="flex justify-end items-center flex-wrap gap-2 pt-4 border-t p-4 bg-muted/20">
          {isUpcoming && onJoinWaitingRoom && onJoinMeeting && (
              <>
            ...
            {!isUpcoming && appointment.status === 'Completado' && !appointment.isReviewed && (
